...
(() => {
  ...
  /* The handshake for the bundles that are not loaded on every page.
   *
   * The map editor and the structured-data editors are only wanted by a page
   * that actually renders a geometry, an array or an hstore, and making every
   * list view download them to find that out is what a size budget exists to
   * prevent. They ship as their own files, requested by the pages that need
   * them, and they reach the pieces of this one through here rather than
   * through a module system -- the admin has no build step, and `script-src
   * 'self'` is happy either way.
   *
   * `register` rather than exposing `enhancers` itself: a companion bundle has
   * to work whether it arrives before or after this file's own first pass over
   * the document. Deferred scripts run in document order so it is always after
   * in practice, but a widget that silently never upgrades is a bad way to find
   * out that assumption changed. Running the pass immediately on registration
   * costs one walk of a scope that is usually a handful of nodes.
   *
   * Not frozen: `toast` and `confirm` are attached further down, once the
   * overlay section that owns them has been defined. */
  window.FastFort = {
    enhance,
    icon,
    el,
